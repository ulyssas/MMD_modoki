...
- `ui-controller.ts` は 2026-04 時点の 7,800 行超から 4,593 行まで減った。
- 行数ベースでは約 3 分の 1 弱を外へ逃がせた。
- 安全に切り出しやすい toolbar / panel / export / accessory / layout はかなり進んだ。
- まだ残っている大きな塊は、bloom / tone mapping / lens / edge などの PostFX、camera controls、model info、timeline / keyframe 編集。
...
- 次に右パネルを続けるなら、project persistence を持たない `LensEffectController` / `EdgePostFxController` が扱いやすい。
