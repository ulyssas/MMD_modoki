...
2026-04-18 時点では、右パネル PostFX、model info、camera controls まで切り出せており、短期の整理はいったん区切れる状態になった。

現在の行数:

```text
src/ui-controller.ts: 4456 行
src/ui/accessory-panel-controller.ts: 444 行
src/ui/bloom-tone-map-controller.ts: 162 行
src/ui/camera-panel-controller.ts: 113 行
src/ui/color-postfx-controller.ts: 208 行
src/ui/dof-panel-controller.ts: 427 行
src/ui/experimental-postfx-controller.ts: 122 行
src/ui/export-ui-controller.ts: 719 行
src/ui/fog-panel-controller.ts: 243 行
src/ui/layout-ui-controller.ts: 434 行
src/ui/lens-effect-controller.ts: 288 行
src/ui/lut-panel-controller.ts: 347 行
src/ui/lut-panel-state.ts: 112 行
src/ui/model-edge-controller.ts: 106 行
src/ui/model-info-panel-controller.ts: 157 行
src/ui/runtime-feature-ui-controller.ts: 280 行
src/ui/scene-environment-ui-controller.ts: 154 行
src/ui/shader-panel-controller.ts: 442 行
```
...
- `ModelInfoPanelController`
  - info model selector、camera / model target 選択、model visibility / delete。
- `CameraPanelController`
  - camera view preset buttons、camera distance slider、camera UI refresh。
- `LayoutUiController`
...
- `LensEffectController`
  - lens distortion / distortion influence / lens edge blur / chromatic aberration、FoV 連動 distortion の auto readout。
- `LutPanelController`
...
- `BloomToneMapController`
  - bloom enabled / weight / threshold / kernel、tone mapping type、hidden glow intensity。
- `ExperimentalPostFxController`
  - hidden motion blur / SSR / volumetric light、SSAO 無効化の初期化。
- `FogPanelController`
  - fog enabled / mode / range / density / opacity / color の static controls。

進捗感:

- `ui-controller.ts` は 2026-04 時点の 7,800 行超から 4,456 行まで減った。
- 行数ベースでは約 43% を外へ逃がせた。
- 安全に切り出しやすい toolbar / panel / export / accessory / layout / model info / camera controls はかなり進んだ。
- 右パネル PostFX の実処理は効果単位の controller 側へ移した。
- model info と camera controls は DOM event / button state / slider sync を controller 側へ移した。
- まだ残っている大きな塊は、右パネル HTML composition、camera selection orchestration、timeline / keyframe 編集。

今回の整理でできたこと:

- `UIController` は、個別 UI の DOM event handler を直接抱える箇所が大きく減った。
- 各 controller は `MmdManager` の runtime 値と DOM を同期し、keyframe dirty や複数 panel 同期は親 callback に戻す形に寄せた。
- PostFX は `ShaderPanelController` に抱えさせず、描画効果ごとの controller に分けた。
- `ModelInfoPanelController` / `CameraPanelController` は、timeline / keyframe の本丸へ踏み込みすぎない範囲で分離した。
...
- `npm.cmd run smoke:launch`
  - `engine=WebGPU` で renderer runtime initialized。

ここで区切る理由:

- これ以上の分割は、DOM event の単純移動よりも orchestration / keyframe 編集 / project state に入りやすい。
- `applyCameraSelectionUI()` / `applyActiveModelSelectionUI()` は bottom panel、timeline selection、shader panel refresh をまたぐため、単純な panel controller へ移すと責務が曖昧になる。
- timeline / keyframe 周辺は MMD 編集体験の中核なので、別途設計メモか小さな作業単位を切ってから進める。

残作業の優先候補:

1. 右パネル HTML composition の整理
   - camera target 時の shader panel 内 PostFX HTML。
   - 実処理は controller 側へ移ったため、次は template / build helper と composition root の境界を整理する。
   - `UIController` に HTML 文字列を残すか、薄い builder に逃がすかは次の判断点。
2. camera selection orchestration の整理
   - `applyCameraSelectionUI()` / `applyActiveModelSelectionUI()` はまだ `UIController` に残る。
   - bottom panel、timeline selection、shader panel refresh の接続が絡むため、controller へ移す場合は別段階にする。
3. `TimelineEditUiController` / `KeyframePanelController`
   - keyframe add / delete / nudge、dirty state、interpolation、bone / morph / camera / accessory keyframe 登録。
   - MMD 編集体験の本丸なので最後に回す。

現時点の方針:

- 短期の UI 分割はここでいったん区切る。
- 次に進めるなら、右パネル HTML composition の抽出か、timeline / keyframe 編集 UI の切り出しを別タスクとして扱う。
- `ShaderPanelController` に PostFX / DoF まで抱えさせない方針は維持する。
- `UIController` は当面 composition root / facade として残し、controller 間 callback と project load/save の橋渡しを担当させる。
- camera / model target 選択後の bottom panel、shader panel、timeline selection 同期は `UIController` に残す。
...
### 実装メモ: ModelInfoPanelController

`src/ui/model-info-panel-controller.ts` を追加し、info panel の model selector と model action buttons を切り出した。

移動済み:

- `info-model-select` の DOM 解決と一覧 refresh。
- camera target option / loaded model option の構築。
- model visibility button の enabled / label 同期。
- model delete button の enabled 同期。
- model visibility toggle。
- active model delete。

`UIController` に残したもの:

- camera / model target 切り替え後の bottom panel 更新。
- active model / camera 選択後の shader panel refresh。
- model visibility 変更時の keyframe dirty 付与。
- model delete 後に camera target へ戻す orchestration。
- shader panel へ渡す info model selector state の bridge。

注意点:

- `ModelInfoPanelController` は selector DOM と action button 操作を担当する。
- `UIController` は target 切り替え後の複数 controller 同期を担当する。
- `MODEL_INFO_CAMERA_SELECT_VALUE` を export し、camera target 判定の magic string を `UIController` 側から外した。

### 実装メモ: CameraPanelController

`src/ui/camera-panel-controller.ts` を追加し、camera controls の DOM event と runtime 値同期を切り出した。

移動済み:

- camera view preset buttons。
- camera distance slider / value label。
- camera distance の runtime refresh。
- camera view button の active / aria-pressed 表示。

`UIController` に残したもの:

- camera 操作後に bottom panel を runtime と同期する接続。
- camera 操作後に camera keyframe dirty を付ける接続。
- DoF auto focus / lens / fog refresh の呼び出し。
- camera VMD / timeline camera keyframe 編集。

注意点:

- `CameraPanelController` は UI 操作を runtime に反映するだけに留める。
- keyframe dirty state は timeline / keyframe 領域の責務なので、callback で `UIController` に戻す。
- frame update 中の camera distance 表示は、`CameraPanelController.refresh(false, displayDistance)` で同期する。
...
- camera target 時に PostFX controller 群を接続する composition。
- project 保存 / 読み込み時の外部 WGSL 連携。

注意点:

- 現時点の `ShaderPanelController` は、camera target では `UIController.renderShaderCameraPostEffectsPanel()` を callback で呼ぶ。
- camera target の PostFX / LUT / DoF は shader panel 内に描画されるが、実処理は `ColorPostFxController`、`LutPanelController`、`DofPanelController` などへ分かれている。
- bundled WGSL file scan と external WGSL snippet validation の下地はあるが、bundled WGSL を preset select に並べる UI 導線はまだ整理途中。
- 次に shader 周辺を進めるなら、WGSL material panel と camera PostFX composition を明確に分ける。

### 現状の shader panel の分け方

2026-04-17 時点では、shader panel は次の 2 モードで動く。

1. model target
   - `ShaderPanelController` が担当する。
   - `shader-model-select` は info panel の model selector と同期する。
   - `shader-preset-select` には `mmdManager.getWgslMaterialShaderPresets()` のうち hidden でない preset を並べる。
   - material list をクリックすると、モデル内の 1 material を選択 / 選択解除する。
   - `Apply Selected` は選択 material、`Apply All` はモデル内全 material に preset を適用する。
   - `Reset` は material 選択中なら selected、未選択なら all を `wgsl-mmd-standard` に戻す。
2. camera target
   - `ShaderPanelController` は `UIController.renderShaderCameraPostEffectsPanel()` を呼ぶだけに留める。
   - `UIController` が HTML composition を持ち、各 PostFX controller を `.shader-postfx-controls` に接続する。
   - DoF controls は実体 DOM を `DofPanelController` が camera panel / shader panel 間で移動する。

現状の境界:

- WGSL material preset / material selection は `ShaderPanelController`。
- camera PostFX の値反映は効果ごとの controller。
- camera PostFX の HTML template はまだ `UIController`。
- external WGSL asset path / text の project bridge は `UIController` と `ShaderPanelController` の間に残る。

次の整理候補:

- `renderShaderCameraPostEffectsPanel()` の HTML template を builder / composition helper に逃がす。
- bundled WGSL file scan 結果を preset select に出すか、別導線にするかを決める。
- external WGSL snippet は実験導線として隔離し、通常 preset 選択と混ざりすぎないようにする。
...
- lens distortion auto readout は `LensEffectController` に移動済み。

注意点:

- DoF controls は camera panel に実体 DOM があり、camera target 時だけ shader panel 内の PostFX controls へ移される。
- `DofPanelController` はこの DOM 移動を担当する。
- PostFX / LUT / model edge / lens / bloom / experimental controls は効果単位の controller に分割済み。
...
#### 7. FogPanelController

対象:

- fog enabled。
- fog mode。
- fog start / end / density / opacity。
- fog color。

理由:

- 右パネル内ではなく static camera / effect controls 側の設定だが、PostFX pipeline に属する。
- hidden experimental controls と同じ controller に入れると責務が曖昧になるため、fog は独立 controller とする。
...
### 実装メモ: LensEffectController

`src/ui/lens-effect-controller.ts` を追加し、レンズ効果系の UI を切り出した。

移動済み:

- static controls 側の `effect-lens-distortion`。
- static controls 側の `effect-lens-distortion-influence`。
- static controls 側の `effect-lens-edge-blur`。
- camera target 時の右パネル `chromatic-aberration`。
- camera target 時の右パネル `distortion-influence`。
- camera target 時の右パネル `lens-edge-blur`。
- FoV 連動 lens distortion の auto readout。

`UIController` に残したもの:

- camera / DoF / PostFX の全体 composition。
- camera frame update 時に auto readout を呼ぶ接続。
- `installRangeNumberInputs` の共通 helper。

注意点:

- `DofPanelController` には focus / f-stop / focal length など被写界深度の編集を残した。
- `LensEffectController` は lens distortion / edge blur / chromatic aberration に限定し、DoF の focus 操作とは分けた。
- static controls と右パネル controls が同じ runtime 値を見るため、片方を触ったらもう片方も同期する。

### 実装メモ: BloomToneMapController

`src/ui/bloom-tone-map-controller.ts` を追加し、bloom / tone mapping / glow の右パネル接続を切り出した。
...
- bloom threshold slider は従来通り、右へ動かすほど threshold を下げて glow 範囲を広げる反転マッピングを維持する。
- glow は hidden 項目だが、描画パイプライン上 bloom / tone mapping と近いためこの controller に置いた。

### 実装メモ: ExperimentalPostFxController

`src/ui/experimental-postfx-controller.ts` を追加し、hidden / experimental な PostFX controls を切り出した。

移動済み:

- hidden motion blur strength。
- hidden SSR strength。
- hidden volumetric light exposure。
- camera PostFX panel 接続時の SSAO 無効化。
...
- SSR は従来通り UI からは有効化せず、入力時も disabled 相当の値に戻す。
- 実験項目は hidden のままなので、挙動変更は入れていない。

### 実装メモ: FogPanelController

`src/ui/fog-panel-controller.ts` を追加し、fog static controls を切り出した。
...
- fog density / opacity。
- fog color RGB。
- runtime state からの fog UI refresh。

`UIController` に残したもの:

- camera UI refresh から `FogPanelController.refresh()` を呼ぶ接続。
- range / number 同期 helper。

注意点:

- fog mode は既存 runtime setter が実質 Exp2 に寄せるため、従来通り初期化時に Exp2 を設定する。
- fog は右パネルの HTML ではなく static controls 側だが、PostFX pipeline の一部として同じタイミングで分割した。
...
ただし、これは最終目標であり、短期では巨大な `UIController` から低リスクな UI 領域を順に逃がす。
2026-04-18 時点では Export / Accessory / ModelInfo / Camera / Shader / DoF / PostFX の主要 UI はかなり外へ出せている。
