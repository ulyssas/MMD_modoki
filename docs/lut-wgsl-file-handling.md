...
- Shader パネルの `種類` ドロップダウンから選択:
  - 組み込みプリセット
  - `wgsl/` 配下で検出された `.wgsl`
  - 現在割り当て中だが `wgsl/` 検出対象外の `.wgsl`（絶対パスなど）
- Shader パネル上部の `WGSL` 行:
  - `Load...` で外部 `.wgsl` を読込
  - `Clear` で解除
- 材質一覧の割り当て表示:
  - 外部 WGSL 割り当て中の材質は `WGSL: <ファイル名>`
  - それ以外はプリセット名
...
- 空ファイル不可
- `fragmentOutputs` を含むコードは不可
- `return` 文を含むコードは不可
- `@fragment` / `@vertex` / `fn ...(` を含む「WGSLモジュール丸ごと」は不可
- `diffuseBase += ...` を含む Toon スニペット形式のみ許可

### 保存動作（現行仕様）

- 外部 WGSL ファイル自体:
  - 外部 WGSL を読み込み済みなら `wgsl/<元ファイル名>` として同梱保存
  - 未読込または `Clear` 済みなら `wgslToonShaderPath = null`
- `.mmdproj.json` に保存される WGSL パス:
  - `effects.wgslToonShaderPath` は単一値
  - 材質ごとの外部 WGSL パスは個別シリアライズしない（現行制約）
